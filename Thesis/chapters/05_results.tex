...
The factor of 32 seems to be independent of input size and are not affected by no power of 2 inputs (figure \ref{fig:cuda_kernel_time_stencil_no_pow2}).
...
This confirms our earlier assumption (section \ref{sec:implementation_theory}) that the 32 thread wide warps might play a role.
...
\begin{figure}[H]
    \centering
    \pgfplotstableread[col sep=comma,]{results_linux.csv}\datatable
    \begin{tikzpicture}
        \begin{axis}[
            xlabel = Benchmark,
            xtick = data,
            xticklabel style={rotate=60, anchor=north east, inner sep=0mm},
            ylabel = Execution time compared to naive,
            ymax = 1.5,
            ybar,
            height=10cm,
            xticklabels from table={\datatable}{bench},
        ]
            \addplot[draw = black, fill = black!15] table[
                x expr  = \coordindex,
                y expr  = \thisrow{optimized_t}/\thisrow{naive_t},
                col sep = comma,
            ]{results_linux.csv};
